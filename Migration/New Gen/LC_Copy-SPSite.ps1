...
#>
    ...
    {
        Save-SPWebToXml -url $subweb.Url -xmlDocument $xmlDocument -parentElement $xmlWeb
    }
...
{
    $xmlListName = $xmlList.title
    $folder = $web.GetFolder($xmlListName)
    $list = $web.Lists[$xmlListName]
    ...
    foreach($xmlFile in $xmlList.file)
    {
        ...
        if($xmlList.title -eq "Pages")
        {
            ...
            if($xmlFile.layout -eq "http://mednet.med.ubc.ca/_catalogs/masterpage/ubcfom_homelayout.aspx, UBC FOM Home Page Layout")
            {
                $newFile.Properties["PublishingPageLayout"] = "$($web.Site.url)/_catalogs/masterpage/HomePageLayout.aspx, HomePage"
                $newFile.Update()
            }
            elseif ($xmlFile.layout -eq "http://mednet.med.ubc.ca/_catalogs/masterpage/ubcfom_inner_1col.aspx, UBC FOM 1 Column Inner Page Layout")
            {
                $newFile.Properties["PublishingPageLayout"] = "$($web.Site.url)/_catalogs/masterpage/OneColumn.aspx, OneColumn"
                $newFile.Update()
            }
            elseif ($xmlFile.layout -eq "https://mednet.med.ubc.ca/_catalogs/masterpage/ubcfom_inner_2col.aspx, UBC FOM 2 Column Inner Page Layout")
            {
                $newFile.Properties["PublishingPageLayout"] = "$($web.Site.url)/_catalogs/masterpage/TwoColumn.aspx, TwoColumn"
                $newFile.Update()
            }
            else
            {
